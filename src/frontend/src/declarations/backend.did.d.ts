/* eslint-disable */

// @ts-nocheck

// This file was automatically generated by @caffeinelabs/bindgen@0.1.0.
// You should NOT make any changes in this file as it will be overwritten.
// Additionally, you should also exclude this file from your linter and/or formatter to prevent it from being checked or modified.

import type { ActorMethod } from '@icp-sdk/core/agent';
import type { IDL } from '@icp-sdk/core/candid';
import type { Principal } from '@icp-sdk/core/principal';

export interface ContactInfo {
  'email' : string,
  'address' : string,
  'phone' : string,
}
export interface Inquiry {
  'id' : bigint,
  'name' : string,
  'productId' : bigint,
  'phone' : string,
}
export interface Product {
  'id' : bigint,
  'weight' : string,
  'name' : string,
  'price' : bigint,
}
export interface _SERVICE {
  'addProduct' : ActorMethod<[string, string, bigint], undefined>,
  'deleteProduct' : ActorMethod<[bigint], undefined>,
  'getContactInfo' : ActorMethod<[], ContactInfo>,
  'getInquiries' : ActorMethod<[], Array<Inquiry>>,
  'getProducts' : ActorMethod<[], Array<Product>>,
  'initializeAdmin' : ActorMethod<[], undefined>,
  'initializeDefaultContactInfo' : ActorMethod<[], undefined>,
  'submitInquiry' : ActorMethod<[string, string, bigint], undefined>,
  'updateContactInfo' : ActorMethod<[string, string, string], undefined>,
  'updateProduct' : ActorMethod<[bigint, string, string, bigint], undefined>,
}
export declare const idlService: IDL.ServiceClass;
export declare const idlInitArgs: IDL.Type[];
export declare const idlFactory: IDL.InterfaceFactory;
export declare const init: (args: { IDL: typeof IDL }) => IDL.Type[];